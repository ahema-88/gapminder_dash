...
```{r}
# Extract gapminder data for year 2007
gap_07 <-  gapminder %>% 
  filter(year==2007)

# Highest Life Expantancy country
...
# Lowest Life Expantancy country
...
# Average Life Expantancy country
average_life_exp_country <- gap_07 %>% 
  summarise(weighted_mean=sum(lifeExp*pop)/sum(pop) )  %>% 
  round(1)

average_life_exp_country <- "Global Average"

# Average Life Expantancy value
...
```{r}
gap_07$average_life_exp_country

value_box(
  title = "Average Life Expantancy",
  value = average_life_exp_value,
  showcase = bsicons::bs_icon("heart"),
  theme = value_box_theme(bg="#3f71ab"),
  p(paste0("(",average_life_exp_country,")")))
```
